...
const AIAssistant = () => {
  ...
  const fetchChatHistory = async () => {
    try {
      ...
      const response = await fetch(
        `${API_URL}/api/docs/${documentId}/chat-history`,
        {
          method: "GET",
          headers: {
  ...
  const sendMessage = async () => {
    if (!prompt.trim() || sending || !documentId) {
      return;
    }
    ...
    try {
      ...
      const response = await fetch(
        `${API_URL}/api/docs/${documentId}/chat`,
        {
          method: "POST",
          headers: {
      ...
      setMessages((prev) => [
        ...prev,
        {
          role: "assistant",
          content:
            data.data?.answer || "No answer generated.",
      ...
      <div className="min-h-full bg-[#0b0e13] text-white p-4 sm:p-6 lg:p-8">
        <div className="flex flex-col items-center justify-center min-h-[70vh] text-center">
          ...
          <p className="mt-2 text-sm text-white/35 max-w-sm">
            Select a document from your workspace to start
            chatting with it.
          </p>
          ...
          {loadingHistory ? (
            ...
            <div className="flex flex-col items-center justify-center py-20 text-center">
              ...
              <p className="mt-2 max-w-md text-sm text-white/35">
                ...
                uploaded document. The AI will answer using
                the document's relevant information.
              </p>
            </div>
          ) : (
